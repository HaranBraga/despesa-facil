    ...
    <div class="app-shell" style="max-width: 1200px; margin: 0 auto; display: block; height: auto; min-height: 100vh;">
      <header class="app-header" style="border-radius: 12px; margin: 16px; position: sticky; top: 16px; z-index: 50;">
        ...
        <div style="display:flex; gap:8px;">
            <button class="btn btn-outline btn-sm" id="btn-settings">Configs WhatsApp</button>
            <button class="btn-icon" id="btn-logout">
      ...
      <main style="padding: 0 16px 40px 16px;" class="gap-16">
        <div class="card" style="background: linear-gradient(135deg, rgba(79,156,249,0.1), rgba(124,58,237,0.1));">
           <p style="margin:0; font-weight:600; color:var(--text-primary)">Bem-vindo, ${user.name}</p>
           <p class="text-sm text-muted">Acompanhe os gastos das empresas vinculadas ao seu escritório.</p>
        </div>
        ...
        <div class="grid-2">
            ...
            <div class="card">
                <div class="section-header">
                    <div class="section-title">Relatório Selecionado</div>
                ...
                <div id="report-container">
                    <div class="empty-state">
                        <p class="empty-sub">Selecione uma empresa para visualizar o relatório.</p>
                    </div>
                </div>
            </div>
        </div>
    ...
    async function loadCompanies() {
        try {
            ...
            container.innerHTML = companies.map(c => `
                <div class="expense-item clickable-company" data-id="${c.id}" style="cursor:pointer; transition: transform 0.2s;">
                    <div class="expense-info">
                        <div class="expense-name">${c.razao_social}</div>
                        <div class="text-xs text-muted">CNPJ: ${c.cnpj} | Dono: ${c.owner_name}</div>
                    </div>
                    <svg width="20" height="20" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><polyline points="9 18 15 12 9 6"></polyline></svg>
                </div>
            `).join('');

            document.querySelectorAll('.clickable-company').forEach(el => {
                el.addEventListener('click', () => {
                    document.querySelectorAll('.clickable-company').forEach(i => i.style.borderColor = '');
        ...
        try {
            ...
            overlay.innerHTML = `
                <div class="modal">
                    ...
                    <div class="modal-title">Configurações de Lembrete</div>
                    <div class="gap-16">
